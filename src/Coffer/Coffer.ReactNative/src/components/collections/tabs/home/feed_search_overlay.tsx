...
function FeedSearchOverlay({
  ...
  const shouldSearch =
    (hasTagValue && searchText.length >= 4) ||
    (hasSimpleValue && searchText.length >= 3);
  ...
  return (
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
        ...
        />
        {!shouldSearch ? (
          <CustomText>Please enter at least 3 characters</CustomText>
        ) : (
          <>
            {hasSimpleValue ? (
              <View style={{ height: 40 }}>
                ...
                >
                  <View style={{ flexDirection: "row" }}>
                    {["ALL", "USERS", "COLLECTIONS", "ITEMS"].map(
                      (title, i) => (
                        <TouchableOpacity
                          key={title}
